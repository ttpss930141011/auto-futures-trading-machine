...
def mock_service_container(mocker: "MockerFixture"):
    ...
    service_container_mock.session_repository = mocker.MagicMock(
        spec=SessionRepositoryInterface, name="SessionRepository"
    )
    service_container_mock.condition_repository = mocker.MagicMock(
        spec=ConditionRepositoryInterface, name="ConditionRepository"
    )
    # Mock the use case if it exists within the container
    service_container_mock.send_market_order_use_case = mocker.MagicMock(
        name="SendMarketOrderUseCase"
    )
...
def test_start_controller_user_not_logged_in(mocker: "MockerFixture", mock_service_container):
    ...
    mock_zmq_instance = mocker.patch(
        "src.app.cli_pfcf.controllers.start_controller.zmq.Context.instance"
    ).return_value
...
def test_start_controller_prerequisites_not_met(mocker: "MockerFixture", mock_service_container):
    ...
    mock_status_checker = mocker.patch(
        "src.app.cli_pfcf.controllers.start_controller.StatusChecker"
    )
    ...
    mock_start_app_use_case.assert_called_once_with(
        logger=mock_service_container.logger, status_checker=mock_status_checker.return_value
    )
...
def test_start_controller_success(mocker: "MockerFixture", mock_service_container):
    ...
    mock_service_container.condition_repository.get_all.return_value = (
        {}
    )  # No conditions for simplicity here
    ...
    mock_order_executor = mocker.patch(
        "src.app.cli_pfcf.controllers.start_controller.OrderExecutor"
    )
    ...
    # mocker.patch('src.app.cli_pfcf.controllers.start_controller.zmq.Context.instance').assert_called_once() # Removed redundant check
    mock_zmq_publisher.assert_called_once_with(
        mocker.ANY, mock_service_container.logger, mock_zmq_instance
    )
    mock_zmq_puller.assert_called_once_with(
        mocker.ANY, mock_service_container.logger, mock_zmq_instance
    )
    mock_tick_producer.assert_called_once_with(
        tick_publisher=mock_zmq_publisher.return_value, logger=mock_service_container.logger
    )
    ...
    mock_service_container.logger.log_info.assert_any_call(
        "Application components initialized successfully with ZeroMQ sockets."
    )
    ...
    assert (
        mock_tick_producer.return_value.handle_tick_data is not None
