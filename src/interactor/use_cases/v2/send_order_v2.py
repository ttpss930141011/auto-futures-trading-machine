...
class SendOrderV2UseCase:
    ...
    def execute(self, input_dto: SendMarketOrderInputDto) -> SendMarketOrderOutputDto:
        ...
        Returns:
            SendOrderOutputDto with result
        """
        try:
            ...
            if result.success:
                self._logger.info(
                    f"Order sent successfully via {self._exchange_api.get_exchange_name()}: "
                    f"{result.order_id}"
                )
                ...
                self._logger.error(
                    f"Order failed via {self._exchange_api.get_exchange_name()}: "
                    f"{result.message}"
                )
                return SendMarketOrderOutputDto(
                    is_send_order=False,
            ...
            self._logger.error(f"Error sending order: {e}")
            return SendMarketOrderOutputDto(
                is_send_order=False,
                order_serial="",
                note=str(e),
