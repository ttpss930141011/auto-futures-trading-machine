...
and initialization of components that communicate via ZeroMQ, 
suitable for running in separate processes.
"""
...
class StartController(CliMemoryControllerInterface):
    ...
    def execute(self) -> None:
        ...
        try:
            ...
            print(
                f"Strategy should connect SUB to {ZMQ_TICK_SUB_CONNECT_ADDRESS} and PUSH to {ZMQ_SIGNAL_PUSH_CONNECT_ADDRESS}"
            )
            print(
                f"OrderExecutor should connect PULL to {ZMQ_SIGNAL_PULL_ADDRESS}"
            )  # Puller binds, Pusher connects
    ...
    def _initialize_components(self) -> None:
        ...
        self.tick_publisher = ZmqPublisher(ZMQ_TICK_PUB_ADDRESS, self.logger, self.zmq_context)
        self.signal_puller = ZmqPuller(ZMQ_SIGNAL_PULL_ADDRESS, self.logger, self.zmq_context)
